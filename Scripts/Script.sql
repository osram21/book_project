...
CREATE TABLE `book_project`.`bookInfo` (
	`b_code`      CHAR(4)     NOT NULL COMMENT '도서코드', -- 도서코드
	`b_sub_code`  INTEGER(2)  NOT NULL COMMENT '도서중복코드', -- 도서중복코드
	`c_name`      VARCHAR(50) NULL     COMMENT '분류', -- 분류
	`b_name`      VARCHAR(20) NULL     COMMENT '도서명', -- 도서명
	`author`      VARCHAR(20) NULL     COMMENT '저자', -- 저자
	`p_code`      CHAR(4)     NULL     COMMENT '출판사코드', -- 출판사코드
	`price`       INTEGER     NULL     COMMENT '가격', -- 가격
	`insert_date` DATE        NULL     COMMENT '도서등록일', -- 도서등록일
	`is_del`      BOOLEAN     NULL     DEFAULT false COMMENT '도서폐기여부' -- 도서폐기여부
)
COMMENT '도서';
...
CREATE TABLE `book_project`.`memberInfo` (
	`m_code`     CHAR(4)     NOT NULL COMMENT '회원코드', -- 회원코드
	`m_pass`     VARCHAR(20) NULL     COMMENT '비밀번호', -- 비밀번호
	`m_name`     VARCHAR(20) NULL     COMMENT '성명', -- 성명
	`m_tel`      CHAR(13)    NULL     COMMENT '연락처', -- 연락처
	`m_zip_code` INTEGER(5)  NULL     COMMENT '우편번호', -- 우편번호
	`m_address`  VARCHAR(50) NULL     COMMENT '주소', -- 주소
	`m_group`    CHAR(1)     NULL     COMMENT '관리자모드', -- 관리자모드
	`is_secsn`   BOOLEAN     NULL     DEFAULT false COMMENT '탈퇴여부' -- 탈퇴여부
)
COMMENT '회원';
...
CREATE TABLE `book_project`.`paymentIO` (
	`no`          INTEGER    NOT NULL COMMENT '출납번호', -- 출납번호
	`b_code`      CHAR(4)    NOT NULL COMMENT '도서코드', -- 도서코드
	`b_sub_code`  INTEGER(2) NULL     COMMENT '도서중복코드', -- 도서중복코드
	`m_code`      CHAR(4)    NOT NULL COMMENT '회원코드', -- 회원코드
	`lend_date`   DATE       NULL     COMMENT '대여일', -- 대여일
	`return_date` DATE       NULL     DEFAULT null COMMENT '반납일' -- 반납일
)
COMMENT '출납';
...
ALTER TABLE `book_project`.`paymentIO`
	MODIFY COLUMN `no` INTEGER NOT NULL AUTO_INCREMENT COMMENT '출납번호';

-- 출판사
CREATE TABLE `book_project`.`publisherInfo` (
	`p_code`     CHAR(4)     NOT NULL COMMENT '출판사코드', -- 출판사코드
	`publisher`  VARCHAR(50) NULL     COMMENT '출판사명', -- 출판사명
	`p_name`     VARCHAR(20) NULL     COMMENT '담당자명', -- 담당자명
	`p_tel`      VARCHAR(13) NULL     COMMENT '연락처', -- 연락처
	`p_zip_code` INTEGER(5)  NULL     COMMENT '우편번호', -- 우편번호
	`p_address`  VARCHAR(50) NULL     COMMENT '주소' -- 주소
)
COMMENT '출판사';
...
CREATE TABLE `book_project`.`bookLend` (
	`b_code`       CHAR(4)    NOT NULL COMMENT '도서코드', -- 도서코드
	`b_sub_code`   INTEGER(2) NOT NULL COMMENT '도서중복코드', -- 도서중복코드
	`is_lending`   BOOLEAN    NULL     DEFAULT false COMMENT '대여여부', -- 대여여부
	`b_lend_count` INTEGER    NULL     DEFAULT 0 COMMENT '총 대여횟수' -- 총 대여횟수
)
COMMENT '도서대여정보';
...
CREATE TABLE `book_project`.`memberLend` (
	`m_code`       CHAR(4) NOT NULL COMMENT '회원코드', -- 회원코드
	`is_posbl`     BOOLEAN NULL     DEFAULT true COMMENT '대여가능여부', -- 대여가능여부
	`delay_count`  INTEGER NULL     DEFAULT 0 COMMENT '연체 횟수', -- 연체 횟수
	`m_lend_count` INTEGER NULL     DEFAULT 0 COMMENT '총 대여권수', -- 총 대여권수
	`m_now_count`  INTEGER NULL     DEFAULT 0 COMMENT '현재 대여권수', -- 현재 대여권수
	`black_date`   DATE    NULL     COMMENT '대여금지일' -- 대여금지일
)
COMMENT '회원대여정보';
...
CREATE TABLE `book_project`.`coden` (
	`c_name` VARCHAR(50) NOT NULL COMMENT '분류', -- 분류
	`c_code` CHAR(2)     NOT NULL COMMENT '코드' -- 코드
)
COMMENT '도서분류';
...
INSERT INTO memberInfo (m_code, m_name, m_tel, m_zip_code, m_address, is_secsn, m_pass, m_group) VALUES
...
select m_code, count(*) from paymentio group by m_code; 
